...
* Next I used the code block to narrow it down to 5 terms per topic, to get an idea of what the topics are
...
* When you run the last topicNames line, you should get a list of five words from Topics 1 to 15 in Console.
...
* Check the Plots tab to see the graph

* Now looking at topics over time
 ...
 * Watch out for errors if a package is not found. In that case, you have to install it:
 `install.packages("packagename")` and then `library(packagename)`

 * Check the Plot tab for the graph

 For 15 topics, I got this plot:

 ![Plot for 15 topics over time](plot15topics.png)

 * Re-ran the script for 8 topics (roughly half). Go back in the code to change the number of topics, then re-run everything.

 Here's what it looks like with 8 topics:
 ![Plot for 8 topics over time](plot8topics.png)
